...
function validateConfigObjectWithPluginsBase(
  ...
  const ensureRegistry = (): RegistryInfo => {
    ...
    for (const diag of registry.diagnostics) {
      let diagPath = diag.pluginId ? `plugins.entries.${diag.pluginId}` : "plugins";
      if (!diag.pluginId && diag.message.includes("plugin path not found")) {
        diagPath = "plugins.load.paths";
      }
      const pluginLabel = diag.pluginId ? `plugin ${diag.pluginId}` : "plugin";
      const message = `${pluginLabel}: ${diag.message}`;
      if (diag.level === "error") {
        issues.push({ path: diagPath, message });
      } else {
        warnings.push({ path: diagPath, message });
      }
    }
  ...
  const validateHeartbeatTarget = (target: string | undefined, targetPath: string) => {
    if (typeof target !== "string") {
      return;
    }
    const trimmed = target.trim();
    if (!trimmed) {
      issues.push({ path: targetPath, message: "heartbeat target must not be empty" });
      return;
    }
    ...
    issues.push({ path: targetPath, message: `unknown heartbeat target: ${target}` });
  };
