...
Repository `NEO-PROTOCOL/neobot` was:
...
```bash
gh repo edit NEO-PROTOCOL/neobot \
  --visibility private \
...
```bash
gh repo view NEO-PROTOCOL/neobot --json collaborators
```
...
```bash
gh api repos/NEO-PROTOCOL/neobot \
  --method PATCH \
...
- `@NEO-PROTOCOL/neobot` → PRIVATE (development, keys)
...
gh repo view NEO-PROTOCOL/neobot --json isPrivate,visibility
