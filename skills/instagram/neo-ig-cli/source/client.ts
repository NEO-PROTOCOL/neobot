...
export class InstagramClient extends EventEmitter {
	...
	public async sendReply(
		...
		try {
			// The API only requires item_id and client_context for replies
			// Create a partial object with only the required fields
			const replyToItem: Partial<DirectThreadFeedResponseItemsItem> | undefined =
				replyToMessage.item_id && replyToMessage.client_context
					? {
							// eslint-disable-next-line @typescript-eslint/naming-convention
							item_id: replyToMessage.item_id,
							// eslint-disable-next-line @typescript-eslint/naming-convention
							client_context: replyToMessage.client_context,
						}
					: undefined;
			...
			await this.ig.entity
				...
				.broadcastText(
					text,
					replyToItem as DirectThreadFeedResponseItemsItem | undefined,
				);
		} catch (error) {
