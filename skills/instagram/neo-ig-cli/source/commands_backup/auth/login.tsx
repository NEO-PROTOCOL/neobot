...
export default function Login({options}: Properties) {
	...
	useEffect(() => {
		const run = async () => {
			// Check for environment variables first (for automated login)
			// eslint-disable-next-line n/prefer-global/process
			const envUsername = process.env.INSTAGRAM_USERNAME;
			// eslint-disable-next-line n/prefer-global/process
			const envPassword = process.env.INSTAGRAM_PASSWORD;

			if (envUsername && envPassword) {
				logger.info('Environment variables found, attempting automatic login...');
				setMessage(`Logging in automatically as @${envUsername}...`);
				const autoClient = new InstagramClient();
				setClient(autoClient);

				try {
					const result = await autoClient.login(envUsername, envPassword, {
						initializeRealtime: false,
					});

					if (result.success) {
						setMessage(`Logged in automatically as @${result.username}`);
						setMode('success');
						return;
					} else if (result.twoFactorInfo) {
						logger.warn('2FA required for automatic login. Manual intervention needed.');
						setMessage(
							'2FA required. Automatic login cannot proceed. Please use manual login.',
						);
						setClient(new InstagramClient());
						setMode('form');
						return;
					} else if (result.checkpointError) {
						logger.warn('Challenge required for automatic login. Manual intervention needed.');
						setMessage(
							'Challenge required. Automatic login cannot proceed. Please use manual login.',
						);
						setClient(new InstagramClient());
						setMode('form');
						return;
					} else {
						logger.error('Automatic login failed', result.error);
						setMessage(
							`Automatic login failed: ${result.error ?? 'Unknown error'}. Falling back to manual login.`,
						);
						setClient(new InstagramClient());
						setMode('form');
						return;
					...
					logger.error('Automatic login error', error);
					setMessage(
						`Automatic login error: ${error instanceof Error ? error.message : String(error)}. Falling back to manual login.`,
					);
					setClient(new InstagramClient());
					setMode('form');
					return;
